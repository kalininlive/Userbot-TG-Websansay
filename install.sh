...
mkdir -p src
# Используем существующий server.js, если он уже есть, иначе создаем
...
connect_account() {
  ...
  for _ in $(seq 1 120); do
    ...
    local url=$(echo "$json" | jq -r '.url // .qr // empty')
...
echo -e "\n\033[1;32m[ИНФО]\033[0m Для подключения нового аккаунта в будущем, используйте скрипт 'qr_wizard.sh'."
